...
class AnimalAP10KDataset(Kpt2dSviewRgbImgTopDownDataset):
    ...
    def _load_coco_keypoint_annotation_kernel(self, img_id):
        ...
        for obj in objs:
            if 'bbox' not in obj:
                continue
            if obj['bbox'] is not None and obj['bbox']['left'] is not None and obj['bbox']['right'] is not None:
                x, y, w, h = obj['bbox']['left']
                xr, yr, wr, hr = obj['bbox']['right']
                x1 = max(0, x)
                y1 = max(0, y)
                x3 = max(0, xr)
                y3 = max(0, yr)
                x2 = min(width - 1, x1 + max(0, w - 1))
                y2 = min(height - 1, y1 + max(0, h - 1))
                x4 = min(width - 1, x3 + max(0, wr - 1))
                y4 = min(height - 1, y3 + max(0, hr - 1))
                if ('area' not in obj or obj['area'] > 0) and x2 > x1 and y2 > y1 and x4 > x3 and y4 > y3:
                    obj['clean_bbox_left'] = [x1, y1, x2 - x1, y2 - y1]
                    obj['clean_bbox_right'] = [x3, y3, x4 - x3, y4 - y3]
                    x5 = min(x1, x3)
                    y5 = min(y1, y3)
                    obj['clean_bbox'] = [x5, y5, x4 - x1, y4 - y1]
                    valid_objs.append(obj)
            elif obj['bbox'] is not None and obj['bbox']['left'] is not None:
                x, y, w, h = obj['bbox']['left']
                x1 = max(0, x)
                y1 = max(0, y)
                x2 = min(width - 1, x1 + max(0, w - 1))
                ...
                if ('area' not in obj or obj['area'] > 0) and x2 > x1 and y2 > y1:
                    obj['clean_bbox'] = [x1, y1, x2 - x1, y2 - y1]
                    valid_objs.append(obj)
            elif obj['bbox'] is not None and obj['bbox']['right'] is not None:
                x, y, w, h = obj['bbox']['right']
                x1 = max(0, x)
                y1 = max(0, y)
                x2 = min(width - 1, x1 + max(0, w - 1))
                y2 = min(height - 1, y1 + max(0, h - 1))
